...
class Net(nn.Module):
    def __init__(self):
        ...
        self.decoder_block1 = nn.Sequential(
            BasicConvTransposeBlock(128, 101, upsample_method=upsample_method),
...
class ModelLoss(nn.Module):
    ...
    def forward(self, input, target, reduction='elementwise_mean'):
        # input_scaled = ((input * 0.5) + 0.5).clamp(0,1) ** 0.2
        # target_scaled = ((target * 0.5) + 0.5).clamp(0,1) ** 0.2
...
def train(args, model, device, train_loader, criterion, optimizer, epoch):
    ...
    if epoch % 1 == 0:
        ...
        save_image(gen_pic, './model01_img/autoencoder_image_{}.png'.format(epoch))
